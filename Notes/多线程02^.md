...
![1591163781941](https://mytyporapicute.oss-cn-guangzhou.aliyuncs.com/1591163781941.png)

### 1.2	线程池-基本原理

​	**概述 :** 

​		提到池，应该能想到的就是水池。水池就是一个容器，在该容器中存储了很多的水。那么什么是线程池呢？线程池也是可以看做成一个池子，在该池子中存储很多个线程。

​	线程池存在的**意义**：

​		系统创建一个线程的成本是比较高的，因为它涉及到与操作系统交互，当程序中需要创建大量生存期很短暂的线程时，频繁的创建和销毁线程对系统的资源消耗有可能大于业务处理是对系统资源的消耗，这样就有点"舍本逐末"了。

​	针对这一种情况，为了提高性能，我们就可以采用**线程池**:

​		线程池创建时是空的。在启动时，会创建大量空闲线程，当我们向线程池提交任务的时，线程池就会启动一个线程来执行该任务。等待任务执行完毕以后，线程并不会死亡，而是再次返回到线程池中称为空闲状态。等待下一次任务的执行。但是如果提交任务时，池子中没有空闲线程，也无法创建新的线程，任务就会排队等待。
...
| 方法名称                                            | 说明                                     |
| --------------------------------------------------- | ---------------------------------------- |
| public static ExecutorService newCachedThreadPool() | 创建一个默认的线程池,上限为int的取值上限 |
...
​	选择合适的线程池类型和参数需要根据具体的业务场景和需求来决定。一般来说，有以下几个原则：

- 如果任务是CPU密集型的，即需要大量计算，没有阻塞，那么可以选择FixedThreadPool，并且**核心线程数**和**最大线程数**设置为最大并行数+1
- 如果任务是IO密集型的，即需要大量等待，有阻塞，那么可以选择CachedThreadPool，并且设置合理的超时时间和队列容量

![线程池多大合适](https://mytyporapicute.oss-cn-guangzhou.aliyuncs.com/%E7%BA%BF%E7%A8%8B%E6%B1%A0%E5%A4%9A%E5%A4%A7%E5%90%88%E9%80%82.png)

- 如果任务是定时执行或者周期执行的，那么可以选择ScheduledThreadPool，并且设置合理的延迟时间和周期时间
- 如果任务是异步执行或者并行执行的，那么可以选择WorkStealingPool，并且设置合理的并行度
- 如果任务是单线程串行执行的，那么可以选择SingleThreadExecutor，并且注意异常处理

### 1.5	自定义线程池-ThreadPoolExecutor
...
![1591165506516](https://mytyporapicute.oss-cn-guangzhou.aliyuncs.com/1591165506516.png)
...
![1591169254280](https://mytyporapicute.oss-cn-guangzhou.aliyuncs.com/1591169254280.png)

### 3.4 并发工具类-ConcurrentHashMap1.8原理

![1591169338256](https://mytyporapicute.oss-cn-guangzhou.aliyuncs.com/1591169338256.png)
