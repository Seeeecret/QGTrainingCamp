...
- 隔离性(Isolation)：数据库系统提供的隔离机制，保证事务在受外部并发操作的**可控影响**下的相对独立地运行
...
- Read committed解决**脏读**的方式是: 只有当B事务提交后，A才能看到B事务修改的数据。B提交前，B事务的操作事务A中都看不到。
- Repeatable Read解决**不可重复读**的方式是: A事务开始时会有数据的缓存，此时事务A中对由A进行的数据修改可见，对由B进行数据的修改不可见。即使数据已经被事务B更新，甚至修改被数据B提交，事务A显示的也是原来的结果，直到A事务提交后，才能看到新数据。
- Serializable解决**幻读**的方式是:一次只能有一个事务在操作一条数据，其它事务若想操作同一条，则会被阻塞，得等。此隔离级别观察到的数据是最真实的。
...
show profile 能在做SQL优化时帮我们了解时间都耗费在哪里。通过 have_profiling 参数，能看到当前 MySQL 是否**支持** profile 操作(想要看profiling文件还是要输入第二条set1才行)：
...
| 分类  | 含义  | 特点  |
| ------------ | ------------ | ------------ |
| 聚集索引(Clustered Index)  | 将数据存储与索引放一块，索引结构的叶子节点保存了行数据  | 必须有，而且只有一个,没有的话会自动建一个隐藏的 |
